...
function checkRoundWin() {
  ...
  // Reset scores pour la prochaine manche
  S.scoreA = 0; S.scoreB = 0;
  ...
  if (winsW >= needed) {
    // Victoire du match — popup immédiat
    const isLastMatch = S.mode === 'solo' || S.matches.filter(m => !m.done).length === 1;
    showPopup(
      `<div style="font-size:48px;margin-bottom:8px">🏆</div>
       <div style="font-size:22px;font-weight:800;color:#1E3A8A;margin-bottom:4px">${winner}</div>
       ${joueursWinner ? `<div style="font-size:14px;color:#6B7280;margin-bottom:12px">${joueursWinner}</div>` : ''}
       <div style="font-size:17px;font-weight:700;margin-bottom:4px">Remporte le match!</div>
       <div style="font-size:13px;color:#9CA3AF;margin-bottom:4px">${winsW} – ${winsL} manches</div>
       <div style="font-size:13px;color:#9CA3AF">${sA} – ${sB} pts (dernière manche)</div>`,
      [{
        label: isLastMatch ? '🎉 Terminer' : '➡️ Match suivant',
        style: 'background:#1E3A8A;color:#fff;font-size:16px;',
        action: () => finishMatch(matchFini)
      }]
    );
  } else {
    // Manche gagnée mais pas encore le match
    showPopup(
      `<div style="font-size:36px;margin-bottom:8px">🎯</div>
       <div style="font-size:19px;font-weight:800;color:#1E3A8A;margin-bottom:4px">${winner}</div>
       ${joueursWinner ? `<div style="font-size:13px;color:#6B7280;margin-bottom:10px">${joueursWinner}</div>` : ''}
       <div style="font-size:16px;font-weight:600;margin-bottom:6px">Remporte la manche ${winsW + winsL}!</div>
       <div style="font-size:14px;color:#6B7280;margin-bottom:2px">Score de la manche : ${sA} – ${sB}</div>
       <div style="font-size:13px;color:#9CA3AF">Manches : ${winsW} – ${winsL}</div>`,
      [{
        label: '▶ Manche suivante',
        style: 'background:#16a34a;color:#fff;font-size:15px;',
        action: () => {
          document.getElementById('undoLog').textContent = 'Manche ' + (winsW + winsL + 1) + ' — Bonne chance!';
        }
      }]
    );
  }
